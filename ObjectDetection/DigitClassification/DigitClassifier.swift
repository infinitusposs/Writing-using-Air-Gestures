...
class DigitClassifier {
  ...
  static func newInstance(completion: @escaping ((Predict<DigitClassifier>) -> ())) {
  ...
  func classify(image: UIImage, completion: @escaping ((Predict<String>) -> ())){
    DispatchQueue.global(qos: .background).async {
      ...
      let maxIndex = results.firstIndex(of: maxConfidence) ?? -1
        let humanReadableResult = "Predicted: \(maxIndex)  Confidence: \(maxConfidence)"
        if maxConfidence == 0.1322653{
            self.result = "No result"
        }
        else{
            self.result = humanReadableResult
        }
